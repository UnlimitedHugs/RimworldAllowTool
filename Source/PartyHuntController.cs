...
namespace AllowTool {
	...
	public static class PartyHuntController {
		...
		public static Gizmo TryGetGizmo(Pawn pawn) {
			...
			if (toggle.disabled) {
				toggle.disabledReason = "IsIncapableOfViolence".Translate(pawn.NameStringShort);
			}
			return toggle;
		}
		...
		public static void DoBehaviorForPawn(JobDriver_Wait driver) {
			...
			var verb = hunter.TryGetAttackVerb(!hunter.IsColonist);
			if (hunter.Faction != null && driver.job.def == JobDefOf.WaitCombat && AllowToolUtility.PawnCapableOfViolence(hunter) && !hunter.stances.FullBodyBusy) {
				// fire at target
				if (hunter.drafter.FireAtWill) {
					// fudge melee range for easier target acquisition
					var weaponRange = verb.verbProps.MeleeRange ? 2 : verb.verbProps.range;
